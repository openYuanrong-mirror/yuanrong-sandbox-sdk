...
| `YR_GATEWAY_ADDRESS` | Optional frontend gateway `host:port` for reverse tunnel routes. Falls back to `YR_SERVER_ADDRESS`. |
...
| `YR_SANDBOX_ROUTER_TLS` | Set `1/true/yes` to use HTTPS for direct SandboxRouter user port URLs. Default: `0`. |
